...
let tuple_of_size_n_expected = (loc, n, sexp) =>
  of_sexp_error(
    sprintf("%s_of_sexp: tuple of size %d expected", loc, n),
    sexp,
  );
...
let record_superfluous_fields = (~what, ~loc, rev_fld_names, sexp) => {
  let fld_names_str = String.concat(List.rev(rev_fld_names), ~sep=" ");
  let msg = sprintf("%s_of_sexp: %s: %s", loc, what, fld_names_str);
  of_sexp_error(msg, sexp);
};

let record_duplicate_fields = (loc, rev_fld_names, sexp) =>
  record_superfluous_fields(
    ~what="duplicate fields",
    ~loc,
    rev_fld_names,
    sexp,
  );
...
let rec record_get_undefined_loop = fields =>
  fun
  | [] => String.concat(List.rev(fields), ~sep=" ")
  | [(true, field), ...rest] =>
    record_get_undefined_loop([field, ...fields], rest)
  | [_, ...rest] => record_get_undefined_loop(fields, rest);

let record_undefined_elements = (loc, sexp, lst) => {
  let undefined = record_get_undefined_loop([], lst);
  let msg =
    sprintf(
...
let record_list_instead_atom = (loc, sexp) => {
  let msg = loc ++ "_of_sexp: list instead of atom for record expected";
  of_sexp_error(msg, sexp);
};
...
let ptag_no_args = (loc, sexp) =>
  of_sexp_error(
    loc ++ "_of_sexp: polymorphic variant does not take arguments",
    sexp,
  );
...
let ptag_takes_args = (loc, sexp) =>
  of_sexp_error(
    loc ++ "_of_sexp: polymorphic variant tag takes an argument",
    sexp,
  );

let nested_list_invalid_poly_var = (loc, sexp) =>
  of_sexp_error(
    loc ++ "_of_sexp: a nested list is an invalid polymorphic variant",
    sexp,
  );

let empty_list_invalid_poly_var = (loc, sexp) =>
  of_sexp_error(
    loc ++ "_of_sexp: the empty list is an invalid polymorphic variant",
    sexp,
  );
