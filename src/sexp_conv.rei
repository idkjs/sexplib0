...
let default_string_of_float: ref(float => string);
...
let write_old_option_format: ref(bool);
...
let read_old_option_format: ref(bool);
...
let sexp_of_option: ('a => Sexp.t, option('a)) => Sexp.t;
...
let sexp_of_pair: ('a => Sexp.t, 'b => Sexp.t, ('a, 'b)) => Sexp.t;
...
let sexp_of_hashtbl:
  ('a => Sexp.t, 'b => Sexp.t, Hashtbl.t('a, 'b)) => Sexp.t;
...
let ref_of_sexp: (Sexp.t => 'a, Sexp.t) => ref('a);
...
let lazy_t_of_sexp: (Sexp.t => 'a, Sexp.t) => lazy_t('a);
...
let option_of_sexp: (Sexp.t => 'a, Sexp.t) => option('a);
...
let pair_of_sexp: (Sexp.t => 'a, Sexp.t => 'b, Sexp.t) => ('a, 'b);
...
let list_of_sexp: (Sexp.t => 'a, Sexp.t) => list('a);
...
let array_of_sexp: (Sexp.t => 'a, Sexp.t) => array('a);
...
module Exn_converter: {
  ...
  let add: (~finalise: bool=?, extension_constructor, exn => Sexp.t) => unit;

  module For_unit_tests_only: {let size: unit => int;};
};
